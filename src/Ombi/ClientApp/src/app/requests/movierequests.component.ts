...
export class MovieRequestsComponent implements OnInit {
    ...
    public provideMovie(request: IMovieRequests) {
        if (request.isProvidedByUser) {
            this.notificationService.warning("Movie Already Provided", "This movie has already been marked as provided by a user.");
            return;
        }

        this.requestService.provideMovie(request.id).subscribe(x => {
            if (x.result) {
                ...
                this.notificationService.success(
                    `You have marked that you have ${request.title}`
                );
            } else {
                this.notificationService.warning(
                    "Provide Movie", 
                    x.message ? x.message : x.errorMessage
                );
            }
        });
    }

    public canProvideMovie(request: IMovieRequests): boolean {
        return !request.isProvidedByUser && 
               !request.available && 
               !request.denied;
    }
    ...
    private setOverride(req: IMovieRequests): void {
        ...
        if (req.isProvidedByUser && !req.providedByUserName) {
            this.requestService.getUserInfo(req.providedByUserId).subscribe(user => {
                req.providedByUserName = user.userName;
            });
        }
    }
